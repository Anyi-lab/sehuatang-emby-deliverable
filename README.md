...
- ✅ 磁链库存反查（帖子页导航面板直接标 ✅在库 / ❓不在库 / ⚠️未校验，避免重复入库）
  - v1.14.0 起**裸磁链也能判**：脚本把磁链所在的页面文本一并送去，服务端从中抠番号；再叠一层**台账番号索引**，
    命中即 0 次 115 请求直接出结果（顺带扛住 MCP 掉线/限频）。合集帖有跨条查重防呆，宁标 ⚠️ 不标假 ✅。
   - v1.15.0 起**批量弹窗每行也带这个徽章**（与左面板同一绘制入口），开弹窗就能看清哪条已在库。
...
- ✅ 监控页操作列常驻（v1.15.4）—— 表格列多，1366/1440 屏下最右的「✅继续 / 重推115 / 整理」被推出屏幕外，
  按钮看不见也点不到。现在操作列 sticky 钉在右侧（窄屏横向滚动时照样看得见），列宽也收窄到 1440px 下零横向溢出；
  顺带修掉 `table{overflow:hidden}`——它让 table 自己成了 sticky 的定位容器，固定列贴不住。
